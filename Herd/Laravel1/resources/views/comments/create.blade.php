...
<div class="max-w-2xl mx-auto">
    <div class="mb-6">
        ...
        <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-gray-100">Create New Comment</h1>
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Add a comment to this task</p>
    </div>
    ...
    <x-bladewind::card>
        <form action="{{ route('tasks.comments.store', $task) }}" method="POST" class="space-y-6">
            ...
            <div>
                <x-bladewind::input label="Author" name="author" value="{{ old('author') }}" required="true" placeholder="Enter your name" />
                @error('author')<p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>@enderror
            </div>

            <div>
                <x-bladewind::textarea label="Comment" name="comment_text" required="true" placeholder="Enter your comment">{{ old('comment_text') }}</x-bladewind::textarea>
                @error('comment_text')<p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>@enderror
            </div>

            @include('partials.form-actions', ['cancelUrl' => route('tasks.comments.index', $task), 'submitText' => 'Create Comment'])
        </form>
    </x-bladewind::card>
</div>
@endsection
