...
<div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-xl shadow-lg p-8 border-l-4 border-indigo-500">
        <div class="flex items-center mb-6">
            <a href="{{ route('projects.tasks.show', [$project, $task]) }}" class="text-indigo-600 hover:text-indigo-700 mr-3">
                <i class="fas fa-arrow-left"></i>
            </a>
            <div>
                <h1 class="text-3xl font-bold text-gray-800">
                    <i class="fas fa-edit text-indigo-600 mr-3"></i>Edit Task
                </h1>
        ...
        <form action="{{ route('projects.tasks.update', [$project, $task]) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="details" id="details" rows="4" required
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition @error('details') border-red-500 @enderror">{{ old('details', $task->details) }}</textarea>
                @error('details')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex space-x-4 pt-4">
                ...
                <a href="{{ route('projects.tasks.show', [$project, $task]) }}" class="flex-1 bg-gray-500 text-white px-6 py-3 rounded-lg hover:bg-gray-600 transition text-center font-medium">
