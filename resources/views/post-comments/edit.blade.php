    ...
    <div class="bg-white rounded-xl shadow-lg p-8 border-l-4 border-indigo-500">
        <div class="mb-6">
            ...
            <h1 class="text-3xl font-bold text-gray-800">
                <i class="fas fa-edit text-indigo-600 mr-3"></i>Edit Comment
            </h1>
            <p class="text-gray-600 mt-2">Post: <span class="font-semibold">{{ $post->title }}</span></p>
        </div>

        <form action="{{ route('posts.comments.update', [$post, $comment]) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="content" id="content" rows="6" required
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition @error('content') border-red-500 @enderror">{{ old('content', $comment->content) }}</textarea>
                @error('content')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex space-x-4 pt-4">
                ...
                <a href="{{ route('posts.comments.show', [$post, $comment]) }}" class="flex-1 bg-gray-500 text-white px-6 py-3 rounded-lg hover:bg-gray-600 transition text-center font-medium">
